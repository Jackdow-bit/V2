# -*- coding: utf-8 -*-
"""Reporte NoK - Streamlit V7. Adaptado desde v5_nok_con_zip.py."""
...
CHATTER_NOK_THRESHOLD_LEVAS = 0.0001
CHATTER_NOK_THRESHOLD_APOYOS = 0.00008
...
def get_resultado(value_str, characteristic_name=None, area=None):
    ...
    if characteristic_name in ['(5) 301-400 UPR', '(9) 301-400 UPR']:
        try:
            ...
            threshold = CHATTER_NOK_THRESHOLD_LEVAS if area in ('Base Circle', 'Lift Area') else CHATTER_NOK_THRESHOLD_APOYOS
            if numeric_val > threshold:
                return 'Nok'
        except (ValueError, TypeError):
            # If conversion to float fails, keep the original rule: not NOK by threshold.
            pass
    return 'Ok'
                        ...
                        if match:
                            ...
                            if len(raw_values) == len(CHATTER_LOBES_CHARACTERISTICS) * 2 * 2:
                                ...
                                for i, char_name in enumerate(CHATTER_LOBES_CHARACTERISTICS):
                                    ...
                                    resultado_val = get_resultado(medicion_val, characteristic_name=char_name, area='Base Circle')
                                ...
                                for i, char_name in enumerate(CHATTER_LOBES_CHARACTERISTICS):
                                    ...
                                    resultado_val = get_resultado(medicion_val, characteristic_name=char_name, area='Lift Area')
                                    extracted_data['chatter_lobes_data_rows'].append({
                                        'Nombre del archivo': pdf_filename,
                                        'Pieza': file_consecutive_number,
                                        'Leva': leva_val,
...
def run_processing_pipeline(pdfs_to_process, summary_collection, collection_errors, progress_bar, status_box, current_file_box, max_workers=8):
    ...
    workers_to_use = min(max(1, int(max_workers)), MAX_WORKERS, num_pdfs_total) if num_pdfs_total else 1
    ...
    with ThreadPoolExecutor(max_workers=workers_to_use) as executor:
        ...
        for future in as_completed(future_to_meta):
            completed_count += 1
            filename = future_to_meta[future][1]
            try:
                result = future.result()
                if result['errors']:
                    processed_pdf_errors.append((filename, "; ".join(result['errors'])))
                else:
                ...
                processed_pdf_errors.append((filename, f"Critical error: {e}"))
            ...
            current_file_box.write(f"📄 Último PDF: {filename}")
    ...
    frames = [df for df in (df_apoyos_final, df_levas_final, df_chatter_lobes_final, df_chatter_apoyos_final) if not df.empty]
    if frames:
        df_master = pd.concat(frames, ignore_index=True)
    else:
        df_master = pd.DataFrame(columns=['Nombre del archivo','Pieza','Leva','Apoyo','Caracteristica','Medicion','Area','Resultado'])

    # Create df_nok by filtering df_master
    df_nok = df_master[df_master.get('Resultado', pd.Series(dtype=str)).eq('Nok')].copy()

    # --- Calculate summary statistics for 'Analisis_Resumen' ---
    total_pieces_analyzed = int(df_master['Pieza'].nunique()) if 'Pieza' in df_master.columns else 0
    pieces_with_nok = df_nok['Pieza'].unique() if 'Pieza' in df_nok.columns else []
    num_nok_pieces = len(pieces_with_nok)
    all_pieces_ids = df_master['Pieza'].unique() if 'Pieza' in df_master.columns else []
    ...
    num_ok_characteristics = int((df_master['Resultado'] == 'Ok').sum()) if 'Resultado' in df_master.columns else 0
    ...
    pieces_rejected_by_specific_nok_char = df_nok.loc[df_nok.get('Caracteristica', pd.Series(dtype=str)).eq(specific_nok_char_to_check), 'Pieza'].unique() if 'Pieza' in df_nok.columns else []
    ...
    nok_characteristics_counts = df_nok['Caracteristica'].value_counts().reset_index() if 'Caracteristica' in df_nok.columns else pd.DataFrame(columns=['Caracteristica', 'Cantidad_NOK'])
    if not nok_characteristics_counts.empty:
    ...
    output_excel_filename = f"Reporte_NoK_V7_{timestamp}.xlsx"
    output_excel_filename_part2 = f"Reporte_NoK_V7_Levas_Parte2_{timestamp}.xlsx"
    ...
    if processed_pdf_errors:
        dfs_to_write_to_main_excel.append((pd.DataFrame(processed_pdf_errors, columns=['Archivo', 'Error']), 'Errores'))
    if collection_errors:
        dfs_to_write_to_main_excel.append((pd.DataFrame(collection_errors, columns=['Archivo', 'Error']), 'Errores_Carga'))
...
st.info("💡 Recomendación: desde Android selecciona un solo ZIP que contenga todos los PDFs. Si tu ZIP supera 200 MB, debes aumentar `server.maxUploadSize` en `.streamlit/config.toml`; Streamlit aplica ese límite antes de ejecutar este código.")

with st.sidebar:
    st.header("⚙️ Configuración")
    workers = st.slider("Trabajadores simultáneos", min_value=1, max_value=min(15, max(1, os.cpu_count() or 1)), value=min(8, max(1, os.cpu_count() or 1)), step=1)
    st.caption("En Streamlit Cloud recomiendo 4–8 para estabilidad. Puedes subirlo si el servidor lo soporta.")
    st.info("Umbral chatter Levas (5) 301-400 UPR: 0.0001")
    st.info("Umbral chatter Apoyos: 0.00008")
    st.info(f"ZIP anidados: hasta {MAX_ZIP_DEPTH} niveles")

uploaded_files = st.file_uploader(
    "📦 Selecciona UN ZIP con todos tus PDFs",
    type=["zip", "pdf"],
    accept_multiple_files=False,
    help=("Para Android, coloca los ~1,300 PDFs dentro de un solo ZIP. El programa admite ZIP anidados hasta 8 niveles y elimina PDFs duplicados por SHA-256."),
)

# Streamlit returns a single UploadedFile when accept_multiple_files=False.
# Normalize it to the list expected by collect_pdf_inputs().
...
if result:
    ...
    st.info("👆 Selecciona un ZIP con tus PDFs y después pulsa **Procesar reportes**.")

# Fin de Reporte NoK Streamlit V7
